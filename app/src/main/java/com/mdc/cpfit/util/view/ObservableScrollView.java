...
public class ObservableScrollView extends NestedScrollView implements Scrollable {
    ...
    protected void onScrollChanged(int l, int t, int oldl, int oldt) {
        ...
        dispatchOnScrollChanged(t, oldt, mFirstScroll, mDragging);
        if (mFirstScroll) {
            mFirstScroll = false;
        }
    ...
    private void dispatchOnScrollChanged(int scrollY, int scrolloldY, boolean firstScroll, boolean dragging) {
        if (mCallbacks != null) {
            mCallbacks.onScrollChanged(scrollY, scrolloldY, firstScroll, dragging);
        }
        if (mCallbackCollection != null) {
            for (int i = 0; i < mCallbackCollection.size(); i++) {
                ObservableScrollViewCallbacks callbacks = mCallbackCollection.get(i);
                callbacks.onScrollChanged(scrollY, scrolloldY, firstScroll, dragging);
            }
        }
    }
